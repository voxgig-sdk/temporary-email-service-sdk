...
describe("TemporaryEmailEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TEMPORARY_EMAIL_SERVICE_TEST_TEMPORARY_EMAIL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local temporary_email_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.temporary_email")))
    local temporary_email_ref01_data = nil
    if #temporary_email_ref01_data_raw > 0 then
      temporary_email_ref01_data = helpers.to_map(temporary_email_ref01_data_raw[1][2])
    end
...
function temporary_email_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TEMPORARY_EMAIL_SERVICE_TEST_TEMPORARY_EMAIL_ENTID")
  ...
  if env["TEMPORARY_EMAIL_SERVICE_TEST_LIVE"] == "TRUE" then
