...
// TemporaryEmailService SDK feature test
//
...
class FtCtrl extends TemporaryEmailServiceControl
{
...
class FtClient
{
    ...
    private TemporaryEmailServiceUtility $utility;

    public function __construct(TemporaryEmailServiceUtility $utility, array $options)
    {
    ...
    public function get_utility(): TemporaryEmailServiceUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TemporaryEmailServiceContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TemporaryEmailServiceContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TemporaryEmailServiceContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TemporaryEmailServiceConfig::make_feature($name);
    ...
    private static function buildUrl(TemporaryEmailServiceSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TemporaryEmailServiceError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TemporaryEmailServiceResponse($fetched);
        }
    ...
    private function fail(TemporaryEmailServiceContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TemporaryEmailServiceContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
