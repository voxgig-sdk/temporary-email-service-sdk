...
// TemporaryEmailService SDK retry feature
...
class TemporaryEmailServiceRetryFeature extends TemporaryEmailServiceBaseFeature
{
    ...
    public function init(TemporaryEmailServiceContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (TemporaryEmailServiceContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(TemporaryEmailServiceContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(TemporaryEmailServiceContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
