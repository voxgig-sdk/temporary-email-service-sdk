...
class TemporaryEmailServiceMakeError
{
    public static function call(?TemporaryEmailServiceContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TemporaryEmailServiceContext([], null);
        }
        $op = $ctx->op ?? new TemporaryEmailServiceOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TemporaryEmailServiceResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TemporaryEmailServiceError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new TemporaryEmailServiceError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof TemporaryEmailServiceError) {
