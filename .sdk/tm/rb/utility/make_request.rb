...
module TemporaryEmailServiceUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = TemporaryEmailServiceResponse.new({})
    ...
    if fetch_err
      ...
      response = TemporaryEmailServiceResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TemporaryEmailServiceResponse.new(fetched)
    else
