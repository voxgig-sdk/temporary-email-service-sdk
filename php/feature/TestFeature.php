...
// TemporaryEmailService SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class TemporaryEmailServiceTestFeature extends TemporaryEmailServiceBaseFeature
{
    ...
    public function init(TemporaryEmailServiceContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (TemporaryEmailServiceContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = TemporaryEmailServiceParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (TemporaryEmailServiceContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(TemporaryEmailServiceContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = TemporaryEmailServiceParam::call($ctx, $k);
