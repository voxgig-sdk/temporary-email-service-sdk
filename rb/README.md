# TemporaryEmailService Ruby SDK



The Ruby SDK for the TemporaryEmailService API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.TemporaryEmail` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = TemporaryEmailServiceSDK.test(testopts, sdkopts)
```
...
raise a `TemporaryEmailServiceError` (a `StandardError` subclass), so wrap
...
```
rb/
├── TemporaryEmailService_sdk.rb       -- Main SDK module
...
The main module (`TemporaryEmailService_sdk`) exports the SDK class
