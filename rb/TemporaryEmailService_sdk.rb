...
require_relative 'TemporaryEmailService_types'


class TemporaryEmailServiceSDK
  ...
  def initialize(options = {})
    ...
    utility = TemporaryEmailServiceUtility.new
    @_utility = utility
    ...
    feature_opts = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = TemporaryEmailServiceHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, TemporaryEmailServiceFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    TemporaryEmailServiceUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => TemporaryEmailServiceError.new(
        "#{op}_allow",
        "TemporaryEmailServiceSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue TemporaryEmailServiceError => err
    ...
    if fetched.is_a?(Hash)
      status = TemporaryEmailServiceHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = TemporaryEmailServiceError.new(
        "graphql_error", "TemporaryEmailServiceSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = TemporaryEmailServiceSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
