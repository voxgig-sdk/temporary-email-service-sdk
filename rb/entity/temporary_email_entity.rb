# TemporaryEmailService SDK TemporaryEmail entity
...
class TemporaryEmailEntity
  ...
  def data_set(args)
    if args
      @_data = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [TemporaryEmail, Hash] the loaded TemporaryEmail; raises TemporaryEmailServiceError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = TemporaryEmailServiceHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
