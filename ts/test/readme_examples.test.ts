...
//     (\`new TemporaryEmailServiceSDK(...)\` / \`TemporaryEmailServiceSDK.test(...)\` become
//     \`TemporaryEmailServiceSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'TemporaryEmailServiceSDK'
...
// rewrite real client construction (\`new TemporaryEmailServiceSDK(...)\` and any
// \`TemporaryEmailServiceSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TemporaryEmailServiceSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TemporaryEmailServiceSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
