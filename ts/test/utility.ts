...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TEMPORARYEMAILSERVICE_TEST_LIVE ||
    'TRUE' === process.env.TEMPORARYEMAILSERVICE_TEST_OVERRIDE
  ) {
  ...
  m.TEMPORARYEMAILSERVICE_TEST_EXPLAIN = process.env.TEMPORARYEMAILSERVICE_TEST_EXPLAIN || m.TEMPORARYEMAILSERVICE_TEST_EXPLAIN
