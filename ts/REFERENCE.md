# TemporaryEmailService TypeScript SDK Reference

Complete API reference for the TemporaryEmailService TypeScript SDK.
...
```ts
new TemporaryEmailServiceSDK(options?: object)
```
...
#### `TemporaryEmailServiceSDK.test(testopts?, sdkopts?)`
...
**Returns:** `TemporaryEmailServiceSDK` instance in test mode.
...
Alias for `TemporaryEmailServiceSDK.test()`.
...
```ts
const result = await client.TemporaryEmail().load({ apikey: 'apikey' })
```
...
Return the parent `TemporaryEmailServiceSDK` instance.
