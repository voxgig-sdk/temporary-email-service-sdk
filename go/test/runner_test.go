...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("TEMPORARYEMAILSERVICE_TEST_LIVE") == "TRUE" ||
		os.Getenv("TEMPORARYEMAILSERVICE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("TEMPORARYEMAILSERVICE_TEST_EXPLAIN"); explain != "" {
