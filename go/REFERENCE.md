# TemporaryEmailService Golang SDK Reference

Complete API reference for the TemporaryEmailService Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *TemporaryEmailServiceSDK`
...
#### `TemporaryEmail(data map[string]any) TemporaryEmailServiceEntity`
...
```go
result, err := client.TemporaryEmail(nil).Load(map[string]any{"apikey": "apikey"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
