// Command TemporaryEmailService-cli is a CLI + REPL for the TemporaryEmailService SDK,
...
const prompt = "temporary-email-service"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TEMPORARY_EMAIL_SERVICE_APIKEY (api key), TEMPORARY_EMAIL_SERVICE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TEMPORARY_EMAIL_SERVICE_APIKEY carries the API key and
	// TEMPORARY_EMAIL_SERVICE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TEMPORARY_EMAIL_SERVICE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TEMPORARY_EMAIL_SERVICE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTemporaryEmailServiceSDK(opts)
