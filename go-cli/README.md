# temporary-email-service-cli

boru-driven command-line client **and** interactive REPL for the TemporaryEmailService
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-service-cli)
make build

# 2. See usage (words, entities, env vars)
./temporary-email-service-cli --help

# 3. Provide credentials once, via the environment
export TEMPORARY_EMAIL_SERVICE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./temporary-email-service-cli load 1 temporary_email            # {id:1} shorthand
./temporary-email-service-cli load '{id:1}' temporary_email       # explicit match map

# 5. Override the API base URL for a single call
TEMPORARY_EMAIL_SERVICE_BASE=https://api.example.com ./temporary-email-service-cli load 1 temporary_email

# 6. No arguments -> interactive REPL
./temporary-email-service-cli
temporary-email-service> load 1 temporary_email
temporary-email-service> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-service-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/temporary-email-service-cli load 1 temporary_email
   ```
...
```sh
./temporary-email-service-cli load 1 temporary_email          # scalar shorthand for {id:1}
./temporary-email-service-cli load '{id:1}' temporary_email     # explicit match map
```
...
./temporary-email-service-cli load 1 temporary_email
```
...
Run with no arguments to open a REPL (prompt `temporary-email-service>`). Each line is
evaluated as its own boru expression:

```text
$ ./temporary-email-service-cli
temporary-email-service> load 1 temporary_email
temporary-email-service> /help
temporary-email-service> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/temporary-email-service-cli`. |
...
(`./temporary-email-service-cli <expr>`) and interactively (the REPL), and expressions compose the
