# temporary-email-service-mcp

[MCP](https://modelcontextprotocol.io) server exposing the TemporaryEmailService SDK as
two agent tools — `temporary-email-service_list` and `temporary-email-service_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/temporary-email-service-mcp)
make build

# 2. Provide credentials via the environment
export TEMPORARY_EMAIL_SERVICE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user temporary-email-service \
  -- /absolute/path/to/temporary-email-service-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./temporary-email-service-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// temporary-email-service_list: first page of records
{ "entity": "temporary_email" }
{ "entity": "temporary_email", "query": { } }

// temporary-email-service_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/temporary-email-service-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user temporary-email-service \
     -- "$PWD"/dist/*/temporary-email-service-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `temporary-email-service_list` and `temporary-email-service_load` tools now appear
   in new sessions. Ask the agent to *"list temporary_email using temporary-email-service"*
   and it calls `temporary-email-service_list` with `{"entity":"temporary_email"}`.
...
```sh
export TEMPORARY_EMAIL_SERVICE_APIKEY=sk_live_xxx            # API key
export TEMPORARY_EMAIL_SERVICE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `temporary-email-service_list` tool
...
### Call the `temporary-email-service_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `temporary-email-service_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `temporary-email-service_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TEMPORARY_EMAIL_SERVICE_APIKEY` | API key sent with every request. |
| `TEMPORARY_EMAIL_SERVICE_BASE` | Optional override of the API base URL. |
...
```sh
./temporary-email-service-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"temporary-email-service_load","arguments":{"entity":"temporary_email","query":{"id":1}}}}'
```
